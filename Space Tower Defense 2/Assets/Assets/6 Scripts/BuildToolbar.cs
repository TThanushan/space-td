...
public class BuildToolbar : MonoBehaviour {
    ...
    public void ShowAllBuildLocationAvailable()
    {
        ...
        foreach (GameObject node in buildManager.allNodes)
        {
            if (node.GetComponent<Node>().turret)
                continue;
            GetCorrespondingBuildLocation(node.transform.position).SetActive(true);
        }
    }

    private GameObject GetCorrespondingBuildLocation(Vector2 nodePosition)
    {
        foreach (GameObject buildLocation in buildLocations)
        {
            if (buildLocation.transform.position.Equals(nodePosition))
