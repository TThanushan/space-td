...
public class UIScript : MonoBehaviour {
    ...
    private void SetStartPlayerStats()
    {
        PlayerStatsScript playerStats = PlayerStatsScript.instance;
        playerStats.money = playerStats.StartMoney;
        playerStats.life = playerStats.StartLife;
    }
    ...
    void PlayerInputs()
    {
        if (Input.GetKeyDown(KeyCode.F))
            showFPS = !showFPS;
        if (!SpawnerScript.instance.WaveIsInProgress() && !PlayerStatsScript.instance.IsGamePaused && Input.GetKeyDown(KeyCode.Space))
        {
            PlaySfx("Wave Start");
            SkipWaveTime();
            PlayAnnounceWaveAnimation();
        }
    }
    ...
    public void SetTimeSpeed(float _speed)
    {
        ...
        Analytics.CustomEvent("Game Speed", new Dictionary<string, object>
            {
                {"Speed", _speed}
            });
    }
