...
public class UISelectedTurretStats : MonoBehaviour
{
    ...
    public void DisplayUpgradeStat()
    {
        ...
        if (IsEffectStatChanging() && towerScript.towerEffect != TowerScript.TowerEffect.noEffect)
            specificUpgradeText.text = "=> " + GetSpecificUpgradeText(towerScript);
        else
            specificUpgradeText.text = string.Empty;
    }

    private bool IsEffectStatChanging()
    {
        TowerScript towerScript = GetNodeUISelectedTowerScript();
        TowerScript upgradeTower = GetUpgradePrefab().GetComponent<TowerScript>();
        if (IsCannonTurret(towerScript))
            return towerScript.bulletG.GetComponent<MissileBullet>().ExplosionRange != upgradeTower.bulletG.GetComponent<MissileBullet>().ExplosionRange;
        else if (IsElectricTurret(towerScript))
            return towerScript.lightningBounceCount != upgradeTower.lightningBounceCount;
        else if (IsChargingTurret(towerScript))
            return towerScript.minAttackspeed != upgradeTower.minAttackspeed;
        return false;
    }
